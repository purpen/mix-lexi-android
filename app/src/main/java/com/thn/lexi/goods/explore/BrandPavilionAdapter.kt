...
class BrandPavilionAdapter(layoutResId: Int) : BaseQuickAdapter<EditorRecommendBean.DataBean.ProductsBean, BaseViewHolder>(layoutResId) {
    override fun convert(helper: BaseViewHolder, item: EditorRecommendBean.DataBean.ProductsBean) {
        ...
        GlideUtil.loadImageWithBlur(R.mipmap.ic_launcher,imageViewBg)

        val imageViewShop = helper.getView<ImageView>(R.id.imageViewShop)
        GlideUtil.loadImage("",imageViewShop)

        helper.setText(R.id.textViewTitle,item.name)
        helper.setText(R.id.textViewCount,"1")

        val imageViewGoods0 = helper.getView<ImageView>(R.id.imageViewGoods0)
        GlideUtil.loadImage(R.mipmap.ic_launcher,imageViewGoods0)
