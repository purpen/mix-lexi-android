package com.thn.lexi.index.explore.newGoods
...
class DialogBottomSynthesiseSort(context: FragmentActivity?, presenter: AllNewGoodsPresenter) : BottomBaseDialog<DialogBottomSynthesiseSort>(context) {
    private lateinit var view: View
    private val present: AllNewGoodsPresenter = presenter
    ...
    private fun setSelection(sortType:String){
        ...
        when (sortType) {
            AllNewGoodsPresenter.SORT_TYPE_SYNTHESISE -> {
            ...
            AllNewGoodsPresenter.SORT_TYPE_LOW_UP -> {
            ...
            AllNewGoodsPresenter.SORT_TYPE_UP_LOW -> {
    ...
    override fun setUiBeforShow() {
        ...
        view.textViewSynthesis.setOnClickListener {
            setSelection(AllNewGoodsPresenter.SORT_TYPE_SYNTHESISE)
            val page = 1
            val minPrice = ""
            val maxPrice = ""
            val cids = ""
            present.loadData(page, AllNewGoodsPresenter.SORT_TYPE_SYNTHESISE, minPrice, maxPrice,cids,"","","","")
            dismiss()
        }

        //价格由低到高
        view.textViewLow2Up.setOnClickListener {
            setSelection(AllNewGoodsPresenter.SORT_TYPE_LOW_UP)
            val page = 1
            val minPrice = ""
            val maxPrice = ""
            val cids = ""
            present.loadData(page, AllNewGoodsPresenter.SORT_TYPE_LOW_UP, minPrice, maxPrice,cids,"","","","")
            dismiss()
        }

        //价格由高到低
        view.textViewUp2Low.setOnClickListener {
            setSelection(AllNewGoodsPresenter.SORT_TYPE_UP_LOW)
            val page = 1
            val minPrice = ""
            val maxPrice = ""
            val cids = ""
            present.loadData(page, AllNewGoodsPresenter.SORT_TYPE_UP_LOW, minPrice, maxPrice,cids,"","","","")
            dismiss()
        }
    }
}