package com.thn.lexi.mine.enshrine.recentLook
...
class DialogBottomSynthesiseSort(context: FragmentActivity?, presenter: AllRecentLookGoodsPresenter) : BottomBaseDialog<DialogBottomSynthesiseSort>(context) {
    ...
    private fun setSelection(sortType:String){
        ...
        when (sortType) {
            AllRecentLookGoodsPresenter.SORT_TYPE_SYNTHESISE -> {
            ...
            AllRecentLookGoodsPresenter.SORT_TYPE_LOW_UP -> {
            ...
            AllRecentLookGoodsPresenter.SORT_TYPE_UP_LOW -> {
    ...
    override fun setUiBeforShow() {
        ...
        view.textViewSynthesis.setOnClickListener {
            setSelection(AllRecentLookGoodsPresenter.SORT_TYPE_SYNTHESISE)
            val page = 1
            val minPrice = ""
            val maxPrice = ""
            val cids = ""
            present.loadData(page, AllRecentLookGoodsPresenter.SORT_TYPE_SYNTHESISE, minPrice, maxPrice,cids,"","","","")
            dismiss()
        }

        //价格由低到高
        view.textViewLow2Up.setOnClickListener {
            setSelection(AllRecentLookGoodsPresenter.SORT_TYPE_LOW_UP)
            val page = 1
            val minPrice = ""
            val maxPrice = ""
            val cids = ""
            present.loadData(page, AllRecentLookGoodsPresenter.SORT_TYPE_LOW_UP, minPrice, maxPrice,cids,"","","","")
            dismiss()
        }

        //价格由高到低
        view.textViewUp2Low.setOnClickListener {
            setSelection(AllRecentLookGoodsPresenter.SORT_TYPE_UP_LOW)
            val page = 1
            val minPrice = ""
            val maxPrice = ""
            val cids = ""
            present.loadData(page, AllRecentLookGoodsPresenter.SORT_TYPE_UP_LOW, minPrice, maxPrice,cids,"","","","")
            dismiss()
        }
    }
}