...
class ArticleDetailCommentListAdapter(res: Int, presenter: ArticleDetailPresenter) : BaseQuickAdapter<CommentBean, BaseViewHolder>(res) {
    private val present: ArticleDetailPresenter = presenter
    private var adapter: ArticleDetailSubCommentListAdapter? = null
    ...
    override fun convert(helper: BaseViewHolder, item: CommentBean) {
        ...
        if (item.sub_comment_count == 0) {
            ...
            adapter = ArticleDetailSubCommentListAdapter(R.layout.adapter_shopwindow_detail_subcomment, present)
