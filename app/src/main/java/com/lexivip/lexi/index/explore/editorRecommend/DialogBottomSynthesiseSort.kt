...
class DialogBottomSynthesiseSort(context: FragmentActivity?, presenter: AllEditorRecommendPresenter) : BottomBaseDialog<DialogBottomSynthesiseSort>(context) {
    ...
    private fun setSelection(sortType:String){
        ...
        when (sortType) {
            AllEditorRecommendPresenter.SORT_TYPE_SYNTHESISE -> {
            ...
            AllEditorRecommendPresenter.SORT_TYPE_LOW_UP -> {
            ...
            AllEditorRecommendPresenter.SORT_TYPE_UP_LOW -> {
    ...
    override fun setUiBeforShow() {
        ...
        view.textViewSynthesis.setOnClickListener {
            setSelection(AllEditorRecommendPresenter.SORT_TYPE_SYNTHESISE)
            val page = 1
            val minPrice = ""
            val maxPrice = ""
            val cids = ""
            present.loadData(page, AllEditorRecommendPresenter.SORT_TYPE_SYNTHESISE, minPrice, maxPrice,cids,"","","","")
            dismiss()
        }

        //价格由低到高
        view.textViewLow2Up.setOnClickListener {
            setSelection(AllEditorRecommendPresenter.SORT_TYPE_LOW_UP)
            val page = 1
            val minPrice = ""
            val maxPrice = ""
            val cids = ""
            present.loadData(page, AllEditorRecommendPresenter.SORT_TYPE_LOW_UP, minPrice, maxPrice,cids,"","","","")
            dismiss()
        }

        //价格由高到低
        view.textViewUp2Low.setOnClickListener {
            setSelection(AllEditorRecommendPresenter.SORT_TYPE_UP_LOW)
            val page = 1
            val minPrice = ""
            val maxPrice = ""
            val cids = ""
            present.loadData(page, AllEditorRecommendPresenter.SORT_TYPE_UP_LOW, minPrice, maxPrice,cids,"","","","")
            dismiss()
        }
    }
}