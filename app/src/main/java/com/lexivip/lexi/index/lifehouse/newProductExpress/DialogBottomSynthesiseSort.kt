...
class DialogBottomSynthesiseSort(context: FragmentActivity?, presenter: NewProductExpressPresenter) : BottomBaseDialog<DialogBottomSynthesiseSort>(context) {
    private lateinit var view: View
    private val present: NewProductExpressPresenter = presenter
    ...
    private fun setSelection(sortType:String){
        ...
        when (sortType) {
            NewProductExpressPresenter.SORT_TYPE_SYNTHESISE -> {
            ...
            NewProductExpressPresenter.SORT_TYPE_LOW_UP -> {
            ...
            NewProductExpressPresenter.SORT_TYPE_UP_LOW -> {
    ...
    override fun setUiBeforShow() {
        ...
        view.textViewSynthesis.setOnClickListener { //综合排序将价格清掉
            setSelection(NewProductExpressPresenter.SORT_TYPE_SYNTHESISE)
            val page = 1
            ...
            present.loadData(page, NewProductExpressPresenter.SORT_TYPE_SYNTHESISE, minPrice, maxPrice,cids,is_free_postage,is_preferential,is_custom_made,sortNewest)
            dismiss()
        }

        //价格由低到高
        view.textViewLow2Up.setOnClickListener {
            setSelection(NewProductExpressPresenter.SORT_TYPE_LOW_UP)
            val page = 1
            val minPrice = present.getMinPrice()
            val maxPrice = present.getMaxPrice()
            val cids = present.getCids()
            ...
            val is_preferential = present.isPreferential()
            val is_custom_made = present.isCustomMade()
            present.loadData(page, NewProductExpressPresenter.SORT_TYPE_LOW_UP, minPrice, maxPrice,cids,is_free_postage,is_preferential,is_custom_made,sortNewest)
            dismiss()
        }

        //价格由高到低
        view.textViewUp2Low.setOnClickListener {
            setSelection(NewProductExpressPresenter.SORT_TYPE_UP_LOW)
            val page = 1
            val minPrice = present.getMinPrice()
            val maxPrice = present.getMaxPrice()
            val cids = present.getCids()
            ...
            val is_preferential = present.isPreferential()
            val is_custom_made = present.isCustomMade()
            present.loadData(page, NewProductExpressPresenter.SORT_TYPE_UP_LOW, minPrice, maxPrice,cids,is_free_postage,is_preferential,is_custom_made,sortNewest)
            dismiss()
        }
    }
}